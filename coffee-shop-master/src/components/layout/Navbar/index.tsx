...
export default function Navbar() {
  ...
  const handleCartClick = () => showCartModal();
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 bg-gray-50 border-b z-10">
        <div className="max-w-screen-lg flex items-center justify-between mx-auto px-4 smd:px-0 py-3">
          ...
          <div className="block sm:hidden">
            <NavButton onClick={handleDrawerOpen} Icon={UserIcon} />
          </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link
              to="/book-table"
              className="hidden sm:block text-gray-500 hover:text-primary font-semibold hover:underline flex items-center gap-1"
            >
              <span>Table</span>
            </Link>
          </div>
      ...
      {/* Drawer Menu (mobile) */}
      <MenuDrawer show={showDrawer} onClose={handleDrawerClose} />
    </>
  );
}
